import {remove, titleCase, getValidPkgName} from "@es-pkg/utils";
import {series, parallel} from "@es-pkg/gulp"
...
import {config, pkg} from "@es-pkg/config";
...
All rights reserved.
    ...
    build({
        entry: config.entry,
        output: {
            path: path.join(process.cwd(), config.iife),
            filename: name,
            ...
            rules: [
                {
                    ...
                    include: config.include.map(item => `${path.join(process.cwd(), item)}`)
                },
                {
                    // 用来匹配 .css 结尾的文件
                    test: /\.css$/,
